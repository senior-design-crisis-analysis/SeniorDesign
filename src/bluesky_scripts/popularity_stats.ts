...
const CONFIG = {
  ...
  BATCH_SIZE: 100, // Process 50 posts at a time
...
// Update engagement counts for a single post
async function updatePostEngagement(uri: string): Promise<{ success: boolean; counts?: any; error?: string }> {
  try {
...
async function updateAllEngagements() {
  ...
  while (hasMore) {
    ...
    for (const post of posts) {
      ...
      const result = await updatePostEngagement(post.uri);
